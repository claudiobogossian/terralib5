...
void te::attributefill::RasterToVector::setInput(
    te::rst::Raster* inRaster, te::da::DataSourcePtr inVectorDsrc,
...
void te::attributefill::RasterToVector::setParams(
    ...
    std::vector<te::stat::StatisticalSummary> statSum, bool texture,
    bool readAll)
{
  m_bands = bands;
  m_statSum = statSum;
...
bool te::attributefill::RasterToVector::run()
{
  ...
  std::auto_ptr<te::da::DataSetAdapter> dsVector(
      te::da::CreateAdapter(dataSetVector.get(), m_inVectorDsType.get()));

  // Verify if mode was checked to calculating.
  bool mode = false;
  std::vector<te::stat::StatisticalSummary>::iterator it_mode =
      std::find(m_statSum.begin(), m_statSum.end(), te::stat::MODE);
  ...
  std::auto_ptr<te::mem::DataSet> outDataset(
  ...
  {
    te::mem::DataSetItem* outDSetItem =
        new te::mem::DataSetItem(outDataset.get());

    std::vector<te::dt::Property*> vecProp =
    ...
    switch(geom->getGeomTypeId())
    {
      ...
      {
        te::gm::MultiPolygon* mPolygon =
            dynamic_cast<te::gm::MultiPolygon*>(geom.get());
        ...
        for(std::size_t n = 0; n < n_geom; ++n)
        {
          te::gm::Polygon* polygon =
              dynamic_cast<te::gm::Polygon*>(mPolygon->getGeometryN(n));
          std::vector<std::vector<double> > tempValues =
              rasterAtt->getValuesFromRaster(*m_inRaster, *polygon, m_bands);
          ...
          for(std::size_t band = 0; band < tempValues.size(); ++band)
          {
            ...
            valuesFromRaster[band].insert(it, tempValues[band].begin(),
                                          tempValues[band].end());
          }
        }
      ...
      {
        te::gm::MultiPoint* mPoint =
            dynamic_cast<te::gm::MultiPoint*>(geom.get());
        ...
        {
          te::gm::Point* point =
              dynamic_cast<te::gm::Point*>(mPoint->getGeometryN(n));
          ...
          for(std::size_t band = 0; band < m_inRaster->getNumberOfBands();
              ++band)
          {
            double value;
            m_inRaster->getValue((int)coord2d.getX(), (int)coord2d.getY(),
                                 value, band);
            ...
            valuesFromRaster[band].insert(it, values.begin(), values.end());
          }
    ...
    if(!isPoint)
    {
      for(std::size_t band = 0; band < valuesFromRaster.size(); ++band)
      {
        ...
        if(percentByArea)
          te::stat::GetPercentOfEachClassByArea(valuesFromRaster[band], resX,
                                                resY, area, summary, contains);
    ...
    std::vector<double>& values)
{
  ...
  const unsigned int inputNCols = inputRaster.getNumberOfColumns();
  const unsigned int inputNRows = inputRaster.getNumberOfRows();
  ...
  const unsigned int nblocksX = inputBand.getProperty()->m_nblocksx;
  const unsigned int nblocksY = inputBand.getProperty()->m_nblocksy;
  const unsigned int blkH = inputBand.getProperty()->m_blkh;
  const unsigned int blkW = inputBand.getProperty()->m_blkw;

  std::set<double> valuesSet;
  unsigned int rasterCol = 0;
  unsigned int rasterRow = 0;
  ...
  for(unsigned int blkY = 0; blkY < nblocksY; ++blkY)
  {
    ...
    blkRowBound = std::min(inputNRows, blkStartRow + blkH);

    for(unsigned int blkX = 0; blkX < nblocksX; ++blkX)
    {
      blkStartCol = blkX * blkW;
      blkColBound = std::min(inputNCols, blkStartCol + blkW);

      for(rasterRow = blkStartRow; rasterRow < blkRowBound; ++rasterRow)
      {
        for(rasterCol = blkStartCol; rasterCol < blkColBound; ++rasterCol)
        {
          inputBand.getValue(rasterCol, rasterRow, centerValue);
...
te::attributefill::RasterToVector::getDataSetType(
    std::vector<std::vector<double> > pixelDistinct)
{
  std::auto_ptr<te::da::DataSetType> outdsType(
      new te::da::DataSetType(*m_inVectorDsType->getResult()));
...
std::vector<te::rp::Texture> te::attributefill::RasterToVector::getTexture(
    te::rst::Raster* rst, te::gm::Geometry* geom, int band, bool readAll)
{
...
bool te::attributefill::RasterToVector::save(
    std::auto_ptr<te::mem::DataSet> result,
    std::auto_ptr<te::da::DataSetType> outDsType)
{
  ...
  if(m_outDsrc->getCapabilities()
         .getDataSetTypeCapabilities()
         .supportsPrimaryKey())
  {
