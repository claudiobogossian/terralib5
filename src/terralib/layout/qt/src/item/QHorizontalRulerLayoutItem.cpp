...
void te::layout::QHorizontalRulerLayoutItem::setPosition( const double& x, const double& y )
{
  setPos(x,y);
}
...
te::layout::Coordinate te::layout::QHorizontalRulerLayoutItem::getPosition()
{
  QPointF posF = pos();
  qreal valuex = posF.x();
  qreal valuey = posF.y();
...
void te::layout::QHorizontalRulerLayoutItem::setPixmap( const QPixmap& pixmap )
{
  _pixmap = pixmap;
...
void te::layout::QHorizontalRulerLayoutItem::paint( QPainter * painter, const QStyleOptionGraphicsItem * option, QWidget * widget /*= 0 */ )
{
  Q_UNUSED( option );
  Q_UNUSED( widget );
  if ( !painter )
...
void te::layout::QHorizontalRulerLayoutItem::drawBackground( QPainter * painter )
{
  if (painter)
  {
