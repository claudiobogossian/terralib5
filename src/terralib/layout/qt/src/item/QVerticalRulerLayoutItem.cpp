...
te::layout::QVerticalRulerLayoutItem::QVerticalRulerLayoutItem( LayoutItemController* controller, LayoutObservable* o ) :
...
void te::layout::QVerticalRulerLayoutItem::setPosition( const double& x, const double& y )
{
  setPos(x,y);
}
...
te::layout::Coordinate te::layout::QVerticalRulerLayoutItem::getPosition()
{
  QPointF posF = pos();
  qreal valuex = posF.x();
  qreal valuey = posF.y();
...
void te::layout::QVerticalRulerLayoutItem::setPixmap( const QPixmap& pixmap )
{
  _pixmap = pixmap;
...
void te::layout::QVerticalRulerLayoutItem::paint( QPainter * painter, const QStyleOptionGraphicsItem * option, QWidget * widget /*= 0 */ )
{
  Q_UNUSED( option );
  Q_UNUSED( widget );
  if ( !painter )
...
void te::layout::QVerticalRulerLayoutItem::drawBackground( QPainter * painter )
{
  if (painter)
  {
