...
::IF NOT EXIST %BZIP2D_LIBRARY% (
  cd %BZIP2_DIR%

  echo | set /p="Installing bzip2... "<nul
  echo bzip2 >> ../build.log
  echo --------------------------- >> ../build.log
  ...
  nmake /f makefile.msc clean >NUL 2>NUL
  nmake /f makefile.msc DEBUG=1 >> ../build.log 2>NUL 
...
set ZLIB_INCLUDE_DIR=%ZL_DIR%;%ZL_DIR%\build
set ZL_LIBRARY=%ZL_DIR%\build\Release\zlib.lib
set ZLD_LIBRARY=%ZL_DIR%\build\Debug\zlibd.lib
set ZLIB_LIBRARIES="debug;%ZLD_LIBRARY%;optimized;%ZL_LIBRARY%"

IF NOT EXIST %ZLD_LIBRARY% (
  ...
  cmake -G "Visual Studio 12 2013 Win64" -DCMAKE_INSTALL_PREFIX=%TERRALIB_DEPENDENCIES_DIR% .. >> ../../cmake.log

  cmake --build . --target clean --config Release >> ../../build.log
  cmake --build . --target clean --config Debug >> ../../build.log

  cmake --build . --target ALL_BUILD --config Release >> ../../build.log
  cmake --build . --target ALL_BUILD --config Debug >> ../../build.log
...
:: ReadLine
::  Baixar o zip. Descompactar em subpasta libreadline
...
IF NOT EXIST %READLINED_LIBRARY% (
  echo | set /p="Installing libreadline... "<nul
  
  cd %READLINE_DIR%

  MSBuild.exe readline-32.sln /p:Configuration=Debug >> ../../build.log
  MSBuild.exe readline-32.sln /p:Configuration=Release >> ../../build.log
...
::IF NOT EXIST %PCRED_LIBRARY% (
  ...
  cmake -G "NMake Makefiles" -DCMAKE_INSTALL_PREFIX=%TERRALIB_DEPENDENCIES_DIR%^
 -DCMAKE_BUILD_TYPE=Release^
 -DBZIP2_INCLUDE_DIR="%DBZIP2_INCLUDE_DIR%"^
 -DBZIP2_LIBRARIES="%BZIP2_LIBRARY%"^
 -DZLIB_INCLUDE_DIR="%ZLIB_INCLUDE_DIR%"^
 -DZLIB_LIBRARY="%ZL_LIBRARY%"^
 -DREADLINE_INCLUDE_DIR="%READLINE_INCLUDE_DIR%"^
 -DREADLINE_LIBRARY="%READLINE_LIBRARY%"^
 -DBUILD_SHARED_LIBS=ON ..\.. >> %PCRE_DIR%\..\cmake.log 2>NUL
 
  nmake >nul 2>nul

  cd %PCRE_DIR%\build\Debug

  cmake -G "NMake Makefiles" -DCMAKE_INSTALL_PREFIX=%TERRALIB_DEPENDENCIES_DIR%^
 -DCMAKE_BUILD_TYPE=Debug^
 -DCMAKE_DEBUG_POSTFIX=d^
 -DBZIP2_INCLUDE_DIR="%DBZIP2_INCLUDE_DIR%"^
 -DBZIP2_LIBRARIES="%BZIP2D_LIBRARY%"^
 -DZLIB_INCLUDE_DIR="%ZLIB_INCLUDE_DIR%"^
 -DZLIB_LIBRARY="%ZLD_LIBRARY%"^
 -DREADLINE_INCLUDE_DIR="%READLINE_INCLUDE_DIR%"^
 -DREADLINE_LIBRARY="%READLINED_LIBRARY%"^
 -DBUILD_SHARED_LIBS=ON ..\.. >> %PCRE_DIR%\..\cmake.log 2>NUL
...
IF NOT EXIST %ICONVD_LIBRARY% (
  echo | set /p="Installing iconv... "<nul
  
  cd %ICONV_DIR%

  MSBuild.exe myIconv.sln /p:Configuration=Debug >> ../../build.log
  MSBuild.exe myIconv.sln /p:Configuration=Release >> ../../build.log
...
IF NOT EXIST %FREEXLD_LIBRARY% (
  cd %FREEXL_DIR%

  IF NOT EXIST "lib" mkdir lib

  echo | set /p="Installing freexl... "<nul
  echo freexl >> ..\build.log
  echo --------------------------- >> ../build.log

  nmake /f makefile.vc clean >NUL 2>NUL

  nmake /f makefile.vc >> ..\build.log 2>NUL
  xcopy freexl.lib lib >NUL
  xcopy freexl.dll lib >NUL
  
  nmake /f makefile.vc clean >NUL 2>NUL

  nmake /f makefile.vc DEBUG=1 >> ..\build.log 2>NUL 
  xcopy freexld.lib lib >NUL
  xcopy freexld.dll lib >NUL
...
::  Proj4 version 4.9.1 
::  Usando projeto NMake, alterações nos arquivos de projeto
...
IF NOT EXIST %PJD_LIBRARY% (
  echo | set /p="Installing proj4... "<nul
  
  cd %PJ_DIR%

  echo proj4 >> ../build.log
  echo --------------------------- >> ../build.log

  echo proj4 >> ../install.log
  echo --------------------------- >> ../install.log

  nmake /f makefile.vc clean >NUL 2>nul
  nmake /f makefile.vc install-all INSTDIR=%TERRALIB_DEPENDENCIES_DIR% >NUL 2>../build.log

  nmake /f makefile.vc clean >NUL 2>nul
  nmake /f makefile.vc DEBUG=1  >NUL 2>../build.log
...
::  Clean  
  ...
  nmake /f makefile.vc BUILD_DEBUG=YES WIN64=YES clean >nul 2>nul
  del /s capi\*.obj >nul 2>nul
  
  nmake /f makefile.vc WIN64=YES >nul 2>..\log
...
::  Clean  
  ...
  nmake /f makefile.vc BUILD_DEBUG=YES WIN64=YES >nul  2>..\log
...
IF NOT EXIST %PNGD_LIBRARY% (
  echo | set /p="Installing libpng... "<nul
  
  cd %PNG_DIR%

  echo libpng >%PNG_DIR%\..\build.log >%PNG_DIR%\..\warning.log
  echo --------------------------- >> %PNG_DIR%\..\build.log
  ...
  cmake -G "NMake Makefiles" -DCMAKE_INSTALL_PREFIX="%PNG_DIR%\deploy"^
 -DCMAKE_BUILD_TYPE="Release"^
 -DPNG_STATIC=OFF^
 -DPNG_TESTS=OFF^
 -DZLIB_INCLUDE_DIR=%ZL_DIR%;%ZL_DIR%\build^
 -DZLIB_LIBRARY=%ZL_LIBRARY% ..\.. >nul 2>nul
 
 nmake install >nul 2>nul
 
 cd %PNG_DIR%\build\Debug
...
::IF NOT EXIST %JPGD_LIBRARY% ( 
echo | set /p="Installing libjpeg... "<nul

cd %JPG_DIR%

echo libjpeg >%JPG_DIR%\..\build.log >%JPG_DIR%\..\warning.log
echo --------------------------- >> %JPG_DIR%\..\build.log
...
::IF NOT EXIST %TIFFD_LIBRARY% ( 
echo | set /p="Installing libtiff... "<nul

cd %TIFF_DIR%\libtiff

echo tiff >%TIFF_DIR%\..\build.log >%TIFF_DIR%\..\warning.log
echo --------------------------- >> %TIFF_DIR%\..\build.log

IF NOT EXIST .\lib mkdir .\lib

nmake /f Makefile.vc DEBUG=1 clean >nul 2>nul
...
::IF NOT EXIST %GTIFFD_LIBRARY% ( 
echo | set /p="Installing libgeotiff... "<nul

cd %GTIFF_DIR%

echo geotiff >%GTIFF_DIR%\..\build.log >%GTIFF_DIR%\..\warning.log
echo --------------------------- >> %GTIFF_DIR%\..\build.log
...
::  Building release
  ...
  nmake /f ms\ntdll.mak lib >%SSL_DIR%\..\build.log 2>%SSL_DIR%\..\warnings.log
...
IF NOT EXIST "%CURLD_LIBRARY%" (
  ...
  nmake /f Makefile.vc mode=dll VC=12 WITH_SSL=dll WITH_ZLIB=dll DEBUG=no MACHINE=x64 >nul 2>..\..\build.log

  nmake /f Makefile.vc mode=dll VC=12 WITH_SSL=dll WITH_ZLIB=dll DEBUG=yes MACHINE=x64 >nul 2>..\..\build.log
...
IF NOT EXIST "%ICUD_LIBRARY%" (
  echo|set /p="Installing icu... "<nul
  
  cd %ICU_DIR%\source\allinone
  
  MSBuild.exe allinone.sln /t:makedata /p:Configuration=Release >nul 2> %ICU_DIR%\..\build.log  
  
  MSBuild.exe allinone.sln /t:common /p:Configuration=Debug >nul 2> %ICU_DIR%\..\build.log 
...
IF NOT EXIST %XERCESD_LIBRARY% (
  ...
  pkgdata --name xercesc_messages_3_1 -O %ICUROOT%\bin64 --mode dll -d . -e xercesc_messages_3_1 res-file-list.txt >nul 2> %XERCESCROOT%\..\build.log
  
  cd %XERCESCROOT%\projects\Win32\VC12\xerces-all  

  MSBuild.exe xerces-all.sln /t:XercesLib /p:Configuration="ICU Release" >nul 2> %XERCESCROOT%\..\build.log 
  MSBuild.exe xerces-all.sln /t:XercesLib /p:Configuration="ICU Debug" >nul 2> %XERCESCROOT%\..\build.log
...
IF NOT EXIST "%XML2D_LIBRARY%" (
  echo | set /p="Installing libxml2... "<nul
  
  cd %XML2_DIR%\win32

  echo libxml2 >> %XML2_DIR%\..\build.log
  echo --------------------------- >> %XML2_DIR%\..\build.log

  nmake /f Makefile.msvc libxml >nul 2>%XML2_DIR%\..\build.log
  nmake /f Makefile.msvc DEBUG=1 libxml >nul 2>%XML2_DIR%\..\build.log
...
IF NOT EXIST %BD_LIBRARY% (
  echo | set /p="Installing boost... "<nul
  
  cd %B_DIR%

  echo boost > %B_DIR%\..\build.log
  echo --------------------------- >> %B_DIR%\..\build.log
...
IF NOT EXIST %PGD_LIBRARY% (
  echo | set /p="Installing libpq... "<nul
  
  cd %PGis_DIR%\src\interfaces\libpq

  echo libpq >%PGis_DIR%\..\build.log >%PGis_DIR%\..\warning.log
  echo --------------------------- >> %PGis_DIR%\..\build.log
...
set HDF4D_LIBRARY=%HDF4C_DIR%\binaries\lib\hdfddlld.lib
set MFHDF_LIBRARY=%HDF4C_DIR%\binaries\lib\mfhdfdll.lib
set MFHDFD_LIBRARY=%HDF4C_DIR%\binaries\lib\mfhdfddlld.lib

IF NOT EXIST %HDF4D_LIBRARY% (
  echo | set /p="Installing hdf4... "<nul
  
  cd %HDF4C_DIR%

  echo hdf4 >%HDF4C_DIR%\..\build.log >%HDF4C_DIR%\..\warning.log
  echo --------------------------- >> %HDF4C_DIR%\..\build.log
...
IF NOT EXIST %NETCDFD_LIBRARY% (
  echo | set /p="Installing netcdf... "<nul
  
  cd %NETCDF_DIR%

  echo NETCDF >%NETCDF_DIR%\..\build.log >%NETCDF_DIR%\..\warning.log
  echo --------------------------- >> %NETCDF_DIR%\..\build.log
  ...
  cmake -G "NMake Makefiles" -DCMAKE_INSTALL_PREFIX=%NETCDF_DIR%\binaries^
 -DCMAKE_BUILD_TYPE=Release^
 -DJPEGLIB_H_INCLUDE_DIR=%JPG_INCLUDE_DIR%^
 -DJPEG_LIB=%JPG_LIBRARY%^
 -DMFHDF_H_INCLUDE_DIR=%HDF4_INCLUDE_DIR%\mfhdf^
 -DBUILD_DLL=ON^
 -DCURL_INCLUDE_DIR=%CURL_INCLUDE_DIR%^
 -DCURL_LIBRARY=%CURL_LIBRARY%^
 -DHDF4_DF_LIB=%HDF4_LIBRARY%^
 -DHDF4_MFHDF_LIB=%MFHDF_LIBRARY%^
 -DENABLE_HDF4=ON^
 -DUSE_HDF4=ON^
 -DUSE_HDF5=OFF^
 -DENABLE_NETCDF_4=OFF ..\.. >nul 2>nul
  ...
  cd %NETCDF_DIR%\build\Debug

  cmake -G "NMake Makefiles" -DCMAKE_INSTALL_PREFIX=%NETCDF_DIR%\binaries^
 -DCMAKE_BUILD_TYPE=Debug^
 -DCMAKE_DEBUG_POSTFIX=d^
 -DJPEGLIB_H_INCLUDE_DIR=%JPG_INCLUDE_DIR%^
 -DJPEG_LIB=%JPGD_LIBRARY%^
 -DMFHDF_H_INCLUDE_DIR=%HDF4_INCLUDE_DIR%\mfhdf^
 -DBUILD_DLL=ON^
 -DCURL_INCLUDE_DIR=%CURL_INCLUDE_DIR%^
 -DCURL_LIBRARY=%CURLD_LIBRARY%^
 -DHDF4_DF_LIB=%HDF4D_LIBRARY%^
 -DHDF4_MFHDF_LIB=%MFHDFD_LIBRARY%^
 -DENABLE_HDF4=ON^
 -DUSE_HDF4=ON^
 -DUSE_HDF5=OFF^
 -DENABLE_NETCDF_4=OFF ..\.. >nul 2>nul
...
:: IF NOT EXIST %SQLITED_LIBRARY% (
  echo | set /p="Installing sqlite... "<nul
  
  cd %SQLITE_DIR%

  echo sqlite >%SQLITE_DIR%\..\build.log >%SQLITE_DIR%\..\warning.log
  echo --------------------------- >> %SQLITE_DIR%\..\build.log
...
IF NOT EXIST %SPLITED_LIBRARY% (
  echo | set /p="Installing spatiallite... "<nul
  
  cd %SPLITE_DIR%

  echo spatiallite >%SPLITE_DIR%\..\build.log >%SPLITE_DIR%\..\warning.log
  echo --------------------------- >> %SPLITE_DIR%\..\build.log
...
IF NOT EXIST %EXPATD_LIBRARY% (
  echo | set /p="Installing expat... "<nul
  
  cd %EXPAT_DIR%

  echo EXPAT >%EXPAT_DIR%\..\build.log >%EXPAT_DIR%\..\warning.log
  echo --------------------------- >> %EXPAT_DIR%\..\build.log
  ...
  cmake -G "NMake Makefiles" -DCMAKE_INSTALL_PREFIX=%EXPAT_DIR%\binaries^
 -DCMAKE_BUILD_TYPE=Release^
 -DCMAKE_INSTALL_PREFIX=%EXPAT_DIR%\binaries ..\.. >nul 2>nul
...
IF NOT EXIST %GDALD_LIBRARY% (
  echo | set /p="Installing gdal... "<nul
  
  cd %GDAL_DIR%

  echo gdal >%GDAL_DIR%\..\build.log >%GDAL_DIR%\..\warning.log
  echo --------------------------- >> %GDAL_DIR%\..\build.log
...
::IF NOT EXIST %QBROWSERD_LIBRARY% (
  echo | set /p="Installing qtpropertybrowser... "<nul
  
  cd %QBROWSER_DIR%

  echo qtpropertybrowser >%QBROWSER_DIR%\..\build.log >%QBROWSER_DIR%\..\warning.log
  echo --------------------------- >> %QBROWSER_DIR%\..\build.log
...
:: set QWT_INCLUDE_DIR=%TERRALIB_DEPENDENCIES_DIR%\include\qtpropertybrowser
:: set QWT_LIBRARY=%TERRALIB_DEPENDENCIES_DIR%\lib\qwt.lib
:: set QWTD_LIBRARY=%TERRALIB_DEPENDENCIES_DIR%\lib\qwtd.lib

::IF NOT EXIST %QWTD_LIBRARY% (
  echo | set /p="Installing qwt... "<nul
  
  cd %QWT_PATH%

  echo qwt >%QWT_PATH%\..\build.log >%QWT_PATH%\..\warning.log
  echo --------------------------- >> %QWT_PATH%\..\build.log
...
IF NOT EXIST %LUACD_LIBRARY% (
  echo | set /p="Installing lua... "<nul
  
  cd %LUAC_DIR%

  echo lua >%LUAC_DIR%\..\build.log >%LUAC_DIR%\..\warning.log
  echo --------------------------- >> %LUAC_DIR%\..\build.log
 ...
 -DREADLINE_INCLUDE_DIR=%READLINE_INCLUDE_DIR%^
 -DREADLINE_LIBRARY=%READLINE_LIBRARY% ..\.. >nul 2>nul
  
  nmake install >nul 2>nul

  cd %LUAC_DIR%\build\Debug
 ...
 -DINSTALL_INCLUDE=%TERRALIB_DEPENDENCIES_DIR%\include\lua^
 -DREADLINE_INCLUDE_DIR=%READLINE_INCLUDE_DIR%^
 -DREADLINE_LIBRARY=%READLINED_LIBRARY% ..\.. >nul 2>nul
