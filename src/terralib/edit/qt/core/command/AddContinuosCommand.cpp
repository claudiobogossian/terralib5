...
te::edit::AddContinuosCommand::AddContinuosCommand(std::vector<Feature*>& items, std::vector<te::gm::Coord2D>& coords, int& currentIndex,
...
void te::edit::AddContinuosCommand::draw()
{
  ...
  if(m_currentIndex >= 0)
    renderer.draw(m_addItems[m_currentIndex]->getGeometry(), true);
  else
    draft->fill(Qt::transparent);
