...
te::da::mem::Raster::Raster(const Raster& rhs)
  : te::rst::Raster(rhs),
    m_deleter(0),
    m_externalBuffer(0)
{
...
void te::da::mem::Raster::open(const std::map<std::string, std::string>& rinfo, te::common::AccessPolicy p)
{
...
// things may have changed since we found out it is a tiled raster!
      m_bands.push_back(new te::da::mem::TiledBand(this, bprop.release(), b));
    }
    else
    {
      m_bands.push_back(new te::da::mem::Band(this, bprop.release(), b));
    }
  }
...
std::map<std::string, std::string> te::da::mem::Raster::getInfo() const
{
  return std::map<std::string, std::string>();
}

std::size_t te::da::mem::Raster::getNumberOfBands() const
{
  return m_bands.size();
}

int te::da::mem::Raster::getBandDataType(std::size_t i) const
{
...
const te::rst::Band* te::da::mem::Raster::getBand(std::size_t i) const
{
  return m_bands[i];
}

te::rst::Band* te::da::mem::Raster::getBand(std::size_t i)
{
  return m_bands[i];
}

const te::rst::Band& te::da::mem::Raster::operator[](std::size_t i) const
{
  return *m_bands[i];
}

te::rst::Band& te::da::mem::Raster::operator[](std::size_t i)
{
  return *m_bands[i];
}

te::dt::AbstractData* te::da::mem::Raster::clone() const
{
  return new Raster(*this);
}

void te::da::mem::Raster::close()
{
...
void te::da::mem::Raster::create(te::rst::Grid* g,
...
// copy raster bands
        ...
        {
          if(isTiled)
            m_bands.push_back(new te::da::mem::TiledBand(this, bands[b], b));
          else
            m_bands.push_back(new te::da::mem::Band(this, bands[b], b));
        }
...
// create bands
        for(std::size_t b = 0; b < bands.size(); ++b)
        {
          ...
          m_bands.push_back(new te::da::mem::Band(this, ibprop, b, externalBuffer));
        }
      }
    }
    else
...
// determine the block data width, height and the number of data blocks in each dimension
      ...
      for(std::size_t b = 0; b < nbands; ++b)
      {
        ...
        if(isTiled)
          m_bands.push_back(new te::da::mem::TiledBand(this, ibprop, b));
        else
          m_bands.push_back(new te::da::mem::Band(this, ibprop, b));
      }
    }
  }
  else
