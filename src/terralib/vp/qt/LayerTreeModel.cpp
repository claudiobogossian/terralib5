...
{
  AbstractLayerTreeItemFactory::finalize();
...
//bool te::vp::LayerTreeModel::insertRows(int row, int count, const QModelIndex & parent)
//{
//
//}

QMimeData* te::vp::LayerTreeModel::mimeData(const QModelIndexList& indexes) const
{
  if(indexes.empty())
    return 0;
...
QStringList te::vp::LayerTreeModel::mimeTypes() const
{
  QStringList mimes;
...
bool te::vp::LayerTreeModel::dropMimeData(const QMimeData* data,
...
bool te::vp::LayerTreeModel::removeRows(int row, int count, const QModelIndex& parent)
{
...
Qt::DropActions te::vp::LayerTreeModel::supportedDropActions() const
{ 
  return Qt::MoveAction | Qt::CopyAction;
}

void te::vp::LayerTreeModel::setCheckable(const bool checkable)
{
  m_checkable = checkable;
}

bool te::vp::LayerTreeModel::isCheckable() const
{
  return m_checkable;
}

void te::vp::LayerTreeModel::add(const te::map::AbstractLayerPtr& layer)
{
  if(layer.get() == 0)
    return;
  ...
  if(litem == 0)
    throw Exception(TR_VP("Could not add layer to the LayerTreeModel!"));
...
void te::vp::LayerTreeModel::dataChangedForDescendants(const QModelIndex& parent)
{
  if(!parent.isValid())
    return;
...
void te::vp::LayerTreeModel::dataChangedForAscendents(const QModelIndex& index)
{ 
  QModelIndex ascendentIndex = parent(index);
  if(parent(index).isValid())
  {
