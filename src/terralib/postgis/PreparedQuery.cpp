...
namespace te
{
  namespace pgis
  {
    template<class D> inline void BindValue(te::pgis::PreparedQuery* pq, std::size_t i, std::size_t propertyPos, const te::dt::Property* p, D* d)
    {
      switch(p->getType())
      {
...
void te::pgis::PreparedQuery::prepare(const te::da::Query& /*query*/, const std::map<std::string, te::dt::Property*>& /*paramTypes*/)
{
  throw Exception(TR_PGIS("Not implemented yet!"));
}

void te::pgis::PreparedQuery::execute(const std::map<std::string, te::dt::AbstractData*>& /*paramValues*/)
{
  throw Exception(TR_PGIS("Not implemented yet!"));
}

te::da::DataSet* te::pgis::PreparedQuery::query(const std::map<std::string, te::dt::AbstractData*>& /*paramValues*/,
