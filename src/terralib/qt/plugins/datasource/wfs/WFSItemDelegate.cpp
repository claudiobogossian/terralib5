...
te::qt::plugins::wfs::WFSItemDelegate::WFSItemDelegate(QStyledItemDelegate* decorated, QObject* parent) :
  te::common::Decorator<QStyledItemDelegate>(decorated, false)
{
  setParent(parent);

  m_icon = QIcon::fromTheme("datasource-wfs");
}

te::qt::plugins::wfs::WFSItemDelegate::~WFSItemDelegate()
{

}

void te::qt::plugins::wfs::WFSItemDelegate::paint(QPainter * painter, const QStyleOptionViewItem & option, const QModelIndex & index) const
{
  ...
  if(item->getType() == "LAYER")
  {
    ...
    if(te::wfs::IsWFSLayer(*li->getLayer()))
    {
