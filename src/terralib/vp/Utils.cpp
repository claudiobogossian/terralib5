...
te::gm::Polygon* te::vp::GetValidPolygon(te::gm::Polygon* gm)
{
  te::gm::Polygon* pol = new te::gm::Polygon(0, te::gm::PolygonType);

  for (size_t k = 0; k < gm->getNumRings(); k++)
  {
    te::gm::LineString* c = (te::gm::LineString*)gm->getRingN(k);

    size_t ct = c->getNPoints();

    te::gm::LineString* l = new te::gm::LinearRing(ct, te::gm::LineStringType);

    for (size_t i = 0; i < ct; i++)
    {
      std::auto_ptr<te::gm::Point> pt(c->getPointN(i));
      l->setPoint(i, pt->getX(), pt->getY());
    }
...
te::gm::Geometry* te::vp::GetValidMultiPolygon(const te::gm::Geometry* g)
{
  if (g == 0)
    return 0;
