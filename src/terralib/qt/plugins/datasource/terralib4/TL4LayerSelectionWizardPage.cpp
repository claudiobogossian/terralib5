...
void te::qt::plugins::terralib4::TL4LayerSelectionWizardPage::setTL4Layers(std::vector<std::string> layers)
{
  ...
  {
    QListWidgetItem* item = new QListWidgetItem(getImage(LAYER), QString::fromUtf8(layers[i]), m_ui->m_layersListWidget, LAYER);
...
void te::qt::plugins::terralib4::TL4LayerSelectionWizardPage::setTL4RasterFiles(std::vector<std::pair<std::string, std::string> > rasterFiles)
{
  ...
  {
    QListWidgetItem* item = new QListWidgetItem(getImage(RASTER), QString::fromUtf8(rasterFiles[i].first) + " (" + tr("Not supported") + "!)", m_ui->m_layersListWidget, RASTER);
    item->setCheckState(Qt::Unchecked);
    item->setFlags(Qt::ItemIsSelectable | Qt::ItemIsEnabled);
    item->setToolTip(QString::fromUtf8(rasterFiles[i].second));
    m_ui->m_layersListWidget->addItem(item);
  }
}
...
void te::qt::plugins::terralib4::TL4LayerSelectionWizardPage::setTL4Tables(std::vector<std::string> tables)
{
  ...
  {
    QListWidgetItem* item = new QListWidgetItem(getImage(TABLE), QString::fromUtf8(tables[i]), m_ui->m_layersListWidget, TABLE);
    item->setCheckState(Qt::Checked);
    m_ui->m_layersListWidget->addItem(item);
  }
}
...
void te::qt::plugins::terralib4::TL4LayerSelectionWizardPage::setTL4Rasters(std::vector<std::string> rasters)
{
  ...
  {
    QListWidgetItem* item = new QListWidgetItem(getImage(RASTER), QString::fromUtf8(rasters[i]), m_ui->m_layersListWidget, RASTER);
    item->setCheckState(Qt::Checked);
    m_ui->m_layersListWidget->addItem(item);
  }
}
